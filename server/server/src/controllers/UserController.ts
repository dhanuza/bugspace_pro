...
export const UserController = {
   ...
   * Body: { name: string, email: string, role: Role, createDirectly?: boolean }
   */
  create: async (req: Request, res: Response) => {
    const { name, email, role, createDirectly } = req.body as {
      name?: string;
      email?: string;
      role?: Role;
    ...
    try {
      ...
      const existing = await userRepo.getByEmail(targetEmail);
      if (existing) {
        return res.status(409).json({ error: "User with this email already exists." });
      }

      if (createDirectly) {
        // Generate temporary password: emailusername@ (e.g. xantigravity.1@)
        const emailUsername = targetEmail.split("@")[0];
        const tempPassword = `${emailUsername}@`;
        ...
        // Save to Firestore with UID and requiresPasswordChange: true
        ...
        const user = await service.create(req.orgId!, { name, email: targetEmail, role }, req.user!.id);
